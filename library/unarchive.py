...
notes:
    - requires C(tar)/C(unzip)/C(unrar) command on target host
...
class RarArchive(object):
    ...
    def files_in_archive(self, force_refresh=False):
        ...
        cmd = '%s lb "%s"' % (self.cmd_path, self.src)
        rc, out, err = self.module.run_command(cmd)
        if rc != 0:
            raise UnarchiveError('Unable to list files in the archive')
    ...
    def unarchive(self):
        cmd = '%s x -y -idc -u "%s" "%s"/' % (self.cmd_path, self.src, self.dest)
        rc, out, err = self.module.run_command(cmd)
        if rc == 10:
            # No files to extract
            rc = 0
    ...
    def can_handle_archive(self):
        if not self.cmd_path:
            return False
        cmd = '%s t "%s"' % (self.cmd_path, self.src)
        rc, out, err = self.module.run_command(cmd)
        if rc == 0:
            return True
        return False
...
def pick_handler(src, dest, module):
    handlers = [TgzArchive, ZipArchive, TarArchive, TarBzipArchive, TarXzArchive, RarArchive]
